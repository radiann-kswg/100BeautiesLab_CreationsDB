...
3. **獣爾騎兵 (ShauErRiders)** — 十二支ベースの獣人型改造人間
...
- **短縮ロケータ（バッジ直リンク）**: `b=<Works_Code>-<バッジ>[/<DB>]`（例: `?b=NTS-57`, `?b=FLI-M16/PrimaryDealer`）。バッジは相関図と同じ `Works_Code` + 作品別 typedef の `$IndexDef.$badge` 宣言（`lib/graph/graph-badge.js`）で組み立て、読み取り側は `c` 形式（work / db / idx）へ解決してから通常経路へ合流します（表示後の URL は `c` 形式へ書き換わる）。DB 省略時は DB カタログ順に走査して最初に一致した DB を採るため、生成側（詳細ヘッダの「短縮リンクをコピー」ボタン）はカタログ先頭以外の DB のときだけ `/<DB>` を付けます（同一キャラの派生 DB でバッジが重なるため）。文法は `lib/viewer-locator.js` の `parseShortLocator()` / `buildShortQueryString()`。`b` と `c` が同時にあるときは正規形の `c` を優先します。
- **相関図の圧縮ロケータ（`r`）**: 相関図（`pages/relations.html`）は `r=[<map>/]<Works_Code>/<段の値...>` を正とします（例: `?r=NTS`, `?r=NTS/100BL`, `?r=shared/FLI/M&f=FLI-M16`）。map は `own` / `shared`（`own` は省略）、作品は `Works_Code`、段の値は軸の `$display.facet.codeFrom`（辞書行の列名。例: `Suit_Code`）で引けた辞書 code、無ければ生値、未設定グループは `-`。フォーカス `f` はインデックスバッジ（`NTS-57`。同じバッジのノードが他にあれば `NTS-57/Db`）。旧 `m` / `d`・旧ノードキーの `f`・`c=` / `s=` は読み取りのみ互換。文法は `lib/relations-locator.js`、値の解決は `pages/relations.js` の `resolveLocators()`。辞書 code の値（`Faction_Code` / `Class_Code` 等）は User が辞書行へ手入力します（自動生成しない）。code の文字種は**英数字と `.` だけ**（`_` / `-` は後続機能の区切りに予約、`~` は `URLSearchParams` が `%7E` へ変えるため不可）。同じスコープ（グローバル辞書 + 作品辞書を連結した `#List_*`）で異なる値が同じ code を持たないこと（`tests/data.facet-codes.test.js` が守る）。
...
- **Node バージョンの下限根拠**: jsdom が依存する undici が `node:worker_threads.markAsUncloneable` を使うため、Node 20 以下では jsdom 系テスト（`pages.characters.*` / `section-renders.relation` ほか）が import 段階で `TypeError` になります。CI の `node-version` を下げないこと（ローカルが新しい Node だと再現せず、すり抜けます）。
