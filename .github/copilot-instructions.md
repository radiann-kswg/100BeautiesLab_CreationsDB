...
- **`AI_Optout` による AI タグ生成 / AI 学習の抑止**: 作品別 `db_meta.json` の `Databases.#DB_<DbName>` 直下、または `#DB_<DbName>._Secondaries[]` の各要素に `"AI_Optout": true` を置くと、`tools/patch-aihints.mjs` が AIHints の書き込み・解析を拒否します（DB レベルは全モードで exit code 2、カテゴリ単位は該当レコードを `skipped-ai-optout`）。緊急時のみ `--force-ai-optout` でバイパス可能です。`pkg/cloudflare/scripts/migrate-aihints.mjs` も DB レベルの `AI_Optout: true` を D1 投入時に遮断します（多層防御。カテゴリ単位は未対応）。AI 学習・LLM 取り込みに対する opt-out 表明シグナルも兼ねる単一フラグで、`DB_Hidden` / `Works_Hidden` と同様にスキーマ非宣言・メタ欠損時はスキップという扱いです。
  - **`AI_Optout` は権利上の可否のみを表す**。「キャラデザ未着手なので AI へ空データを渡したくない」は別軸で、`Progress: notProceeded`（付与不要 = soft skip、`--include-not-proceeded` で対象化）と画像ゲート（`skipped-no-image`）が担います。「付与不可（権利）」と「付与不要（充填状況）」を混同してフラグを流用しないでください。
...
- リポジトリ直下の `README.LOCAL.md` は `.gitignore` 対象（Git 管轄外）の**ローカル専用メモファイル**です。本体ローカル・サブローカルそれぞれのクローンごとに個別に存在し、commit / push / pull では他ローカルに共有されません。
- **用途**: そのローカル固有の情報（物理パス、現在チェックアウト中のブランチ・用途、作業中/一時中断中のタスク、次に自分（または別ローカル）が引き継ぐ際の注意点など）を、そのローカルを使う人・エージェント自身のための備忘録として記録します。
- **`_work_in_progress/` との使い分け**: 複数ローカル横断で共有すべき正式な進捗・決定事項は引き続き `_work_in_progress/`（Git 管理下）に記録します。`README.LOCAL.md` はそれを補う「そのローカルだけのメモ」であり、正式な進捗ログの代替にはしません。
- **ファイルが無い場合**: 存在しなければ最低限のテンプレート（ローカルパス等）を作成して構いませんが、パス以外の作業メモ内容は User が手動追記する前提とし、Copilot が創作内容や未確認の推測を書き込まないでください。
- **定時スケジュールによる自動議事録（承認済み運用）**: 毎日 4:00（JST）にスケジュールタスク `local-readme-minutes` が各ローカルの `README.LOCAL.md` へ「定時議事録」を追記します。判断根拠は各ローカルの git 事実（`git status` の未コミット変更 / `git reflog` のブランチ切替 / `git log` の直近コミット / 現在ブランチ）に限定し、**その回に実際に使用された（差分・新規コミット・ブランチ切替のある）ローカルだけ**を更新します。未使用ローカルはファイルを一切変更しません。更新対象は各ローカルの `README.LOCAL.md` のみで、他ファイルの編集・git 書き込み（commit/push/merge）は行いません。本項は上記「ファイルが無い場合」の User 手動追記原則に対する例外として自動追記を認めるものですが、記載は git 事実に基づく下書きである旨を明記し、創作内容・未確認の推測は書きません（最終確定は User が加筆修正します）。
- **配布方法**: 本節は `.github/copilot-instructions.md` / `CLAUDE.md` の両方へ記載し、`README.LOCAL.md` 自体は各ローカルにコミット不要（`.gitignore` 済み）で運用します。
