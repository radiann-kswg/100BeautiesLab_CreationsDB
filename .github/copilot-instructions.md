...
### README.md のライセンス・ガイドライン部分について

**⚠️ 重要**: `README.md`の以下の部分は**ユーザーが手動で管理**するため、GitHub Copilot や AI ツールは**編集してはいけません**：

- `# 一次創作作品におけるガイドライン(日本語版)`セクション
- `# Primary Works Guideline (English Version)`セクション
- これらのセクションに含まれるライセンス情報、利用規約、違反行為の定義など

**理由**: これらの部分は法的・権利的に重要な内容であり、自動更新による意図しない変更を防ぐため、必ずユーザーの手動確認・更新が必要です。

### 更新対象となる部分

以下の技術的な部分のみ、GitHub Copilot や AI ツールによる更新が許可されます：

- `# 当リポジトリについて(日本語版)`以降のテクニカルセクション
- `# About This Repository (English Version)`以降のテクニカルセクション
