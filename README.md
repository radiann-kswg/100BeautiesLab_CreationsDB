...
### 設計上の要点（このリポジトリで解いた問題）

**GitHub Pages（静的ホスティング・サーバーサイド処理なし）の上で、REST風のAPIを成立させる**
というのが本リポジトリの技術的な中心です。

| 課題 | 取った手段 |
| --- | --- |
| 静的ホスティングにAPIサーバーを置けない | Service Worker でリクエストを傍受し、`data/` 配下のJSONから応答を組み立てる擬似APIを実装 |
| 用途の違う3つの利用者に同じデータを出したい | `/api/v1/*`（標準）・`/pages/v1/*`（UI向け・エンリッチメント付）・`/svc/v1/*`（広告ブロッカー回避ミラー）の**3スコープ**を出し分け |
| スコープごとにService Workerが増えて重複する | `lib/sw-common.js`（`StandardEndpointHandlers`）・`lib/data-common.js`（`EnrichmentProcessor` / `ImageProcessor`）に共通化 |
| 手書きJSONの構造がばらつく | `db_type.json` で型を定義し、`$VarsDef` / `$DefType` を専用エンドポイント（`/varsdef` `/typedef` `/defs`）で公開。定義自体を参照可能にした |
| DB間・作品間で情報を相互参照したい | `_DBLink` フィールドを検出して他DB・他作品のレコードを動的に解決する参照解決エンジンを実装 |
| データ量が増えると初期表示が重い | Intersection Observer による画像の遅延読み込み、検索入力のデバウンス、Service Worker のキャッシュ戦略 |
| 利用者の種類ごとに知りたいことが違う | 閲覧者向けガイド / 編集者向け更新ガイドライン / 第三者ポリシー / CONTRIBUTING / テスト手順を**分離**して整備し、主要文書は**日英2言語**で維持 |

### AI協働開発の内訳

#### AIに任せた部分

- Service Worker のエンドポイント実装の下書きと、スコープ間の共通化リファクタリング
- SASS のレスポンシブ記述、UI の細かな調整
- ドキュメントの英訳と整形
- 大量のJSONレコードに対する形式チェック・一括変換スクリプト

#### 自分で判断した部分

- **静的ホスティング上でAPIを成立させるという方針そのもの** — サーバーを借りずに、
  データを機械可読な形で公開する手段としてService Workerを選んだ
- **スキーマ設計** — どの項目を必須とし、どの項目を任意とするか。型定義を
  データと同じリポジトリに置き、APIから引けるようにする構成
- **3スコープに分けた理由** — UI向けのエンリッチメントを標準APIに混ぜない。
  外部から使う人が素のデータを取れる経路を残すための分離
- **参照解決の仕様（`_DBLink`）** — 循環参照や欠損参照をどう扱うかを含めた設計
- **ライセンスと利用条件の設計** — CC BY-NC 4.0 を基礎に、AI学習の可否を含む
  利用条件を明文化し、日英2言語で公開・維持する運用
- **権利情報をREADMEの冒頭に置く構成** — 技術情報より先にガイドラインを読ませる。
  公開データベースの運営者としての優先順位
