...
function buildEnumMaps(...metas) {
    ...
    for (const meta of metas) {
        ...
        for (const [key, val] of Object.entries(varsDef)) {
            ...
            // $Def_*内部の#List_*も対応
            if (/^\$Def_/.test(key) && typeof val === "object" && val !== null) {
                for (const [listKey, listVal] of Object.entries(val)) {
                    if (/^#List_/.test(listKey) && Array.isArray(listVal)) {
                        const mapName = listKey.replace(/^#List_/, '');
                        enumMaps[mapName] = enumMaps[mapName] || {};
                        for (const item of listVal) {
                            const baseField = Object.keys(item).find(f =>
                                !f.endsWith('_JP') && !f.endsWith('_EN')
                            );
                            if (baseField && item[baseField]) {
                                enumMaps[mapName][item[baseField]] = {
                                    JP: item[baseField + '_JP'] || item[baseField],
                                    EN: item[baseField + '_EN'] || item[baseField]
                                };
                            }
                        }
                    }
...
function resolveEnums(obj, enumMaps, statsEnumMaps, parentKey = "", parentObj = null) {
    ...
    for (const [k, v] of Object.entries(obj)) {
        if (k.startsWith('_')) {
            result[k] = `[仕様] ${v}`;
        } else if (enumMaps[k] && typeof v === "string" && enumMaps[k][v]) {
            // 列挙体（MaterialType, KinematicOrStatic, RoleType など）和英併記
            result[k] = {
                value: v,
            ...
            // 配列の場合、要素ごとに再帰（MaterialType, ActionType などの配列も対応）
            result[k] = v.map(item => {
                if (typeof item === "object" && item !== null) {
                    // オブジェクト要素の場合は再帰
                    return resolveEnums(item, enumMaps, statsEnumMaps, k, obj);
                } else if (enumMaps[k] && enumMaps[k][item]) {
                    // 配列要素が列挙体の場合
                    return {
                        value: item,
                        jp: enumMaps[k][item].JP || enumMaps[k][item].jp,
                        en: enumMaps[k][item].EN || enumMaps[k][item].en
                    };
                } else {
                    return item;
                }
