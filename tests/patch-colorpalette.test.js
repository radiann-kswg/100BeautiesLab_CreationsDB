...
describe("detectSwatchChips — 設定画のカラーチップ検出", () => {
  it("Num 4 の設定画から、カタログに印字された 5 色すべてを検出する", () => {
    const img = decodePng(
      fs.readFileSync(path.join(IMAGES, "concept", "cnsp_imgNTS-4.png")),
    );
    ...
    for (const printed of NUM4_PRINTED_CODES) {
      const nearest = Math.min(
        ...chips.map((c) => colorDistance(c.hex, printed)),
      );
  ...
  it("チップが重なって描かれていても個別に検出する（Num 48）", () => {
    ...
    const img = decodePng(
      fs.readFileSync(path.join(IMAGES, "concept", "cnsp_imgNTS-48.png")),
    );
    const chips = detectSwatchChips(img);
    expect(chips.length).toBeGreaterThanOrEqual(5);
  });
  ...
  it("大小が不揃いでも小さなチップを取りこぼさない（Num 75 の青は半径 3.7px しかない）", () => {
    const img = decodePng(
      fs.readFileSync(path.join(IMAGES, "concept", "cnsp_imgNTS-75.png")),
    );
    const chips = detectSwatchChips(img);
    // パレット領域を特定してから、その周辺だけを緩い条件で再捜査することで拾える
    const hasSmallBlue = chips.some(
      (c) => colorDistance(c.hex, "#6F94C8") < 20,
    );
  ...
  it("チップが無い画像では空配列を返す", () => {
    const img = decodePng(
      fs.readFileSync(
        path.join(IMAGES, "corefolder", "1", "emstk_corefolderNTS-1-1.png"),
      ),
    );
    const chips = detectSwatchChips(img);
...
describe("measurePaletteCoverage — 配色の被覆率実測（Role の根拠）", () => {
  it("指定した配色ごとの被覆率を返し、合計が 1 を超えない", () => {
    ...
    const coverage = measurePaletteCoverage(img, [
      "#ED5D47",
      "#FF8682",
      "#FFAC8F",
    ]);
...
describe("rankChipsByCoverage — 測定できない場合のフォールバック", () => {
  it("キャラ画像が無ければ、設定画上のチップの大きさ順にする（coverage は null）", () => {
    ...
    const { ordered, measuredOn } = rankChipsByCoverage(
      chips,
      { Images: {} },
      NTS_WORK,
      IMAGES,
...
describe("upsertColorPaletteInRecord / removeColorPaletteFromRecord", () => {
  ...
  it("既存の ColorPalette があれば値だけを差し替える（キーの位置は動かさない）", () => {
    ...
    const { text: out, mode } = upsertColorPaletteInRecord(
      text,
      spans[0],
      palette,
    );
    ...
    expect(keys.indexOf("ColorPalette")).toBe(
      keys.indexOf("AppearanceDetail") + 1,
    );
...
describe("parseChipList / 手入力チップ — 自動検出できないレコードの受け口", () => {
  it("カラーコードを大文字・# 付きへ正規化する（既存データの表記に合わせる）", () => {
    expect(parseChipList("#67bdbd,#a4daef,#387eb6")).toEqual([
      "#67BDBD",
      "#A4DAEF",
      "#387EB6",
    ]);
  ...
  it("手入力チップも被覆率の降順で Role が決まる（自動検出と同じ扱い）", () => {
    ...
    const record = {
      Images: { corefolder_PNGPath: ["40/emstk_corefolderNTS-40-1"] },
    };
    const { ordered, measuredOn } = rankChipsByCoverage(
      chips,
      record,
...
describe("extractSolidColors — 透過イラストからの配色抽出", () => {
  ...
  it("共通造形色（コアフォルダの毛 #FFFFFF）を除外する", () => {
    ...
    const without = extractSolidColors([img], { exclude: common }).map(
      (c) => c.hex,
    );
  ...
  it("白が主体のキャラでも共通色除外で配色が空にならない（SemiPrimary 222）", () => {
    const file = path.join(
      ...
      "corefolder",
      "222",
      "emstk_corefolderNTS-222A,NTS-222B-1.png",
    );
...
describe("isTransparentArtwork — 素材の自動判別", () => {
  it("コアフォルダ（透過キャラ単体）を透過素材と認識する", () => {
    expect(isTransparentArtwork(decodePng(fs.readFileSync(COREFOLDER_1)))).toBe(
      true,
    );
  });
  ...
  it("背景付きの設定画は透過素材と認識しない", () => {
    const img = decodePng(
      fs.readFileSync(path.join(IMAGES, "concept", "cnsp_imgNTS-1.png")),
    );
...
describe("listImageFields / readCommonColors — スキーマ由来の宣言読み取り", () => {
  it("作品別 db_type.json の Images 宣言からフォルダ名を導く", () => {
    ...
    expect(nts).toContainEqual({
      field: "corefolder_PNGPath",
      folder: "corefolder",
      isList: true,
      paletteSource: "artwork",
    });
    expect(nts).toContainEqual({
      field: "concept_PNGName",
      folder: "concept",
      isList: false,
      paletteSource: "swatch",
    });

    // 作品ごとにフィールド名が違っても、ツール側に名前を書かずに扱える
    expect(listImageFields(UBL_WORK).map((f) => f.field)).toContain(
      "keycapper_PNGPath",
    );
  });
  ...
  it("宣言の無い作品では空配列（除外なしで動く）", () => {
    expect(
      readCommonColors(path.join(REPO_ROOT, "data", "Works_ShouArRiders")),
    ).toEqual([]);
  });
});
...
describe("resolvePaletteImageFields — 検出対象の typedef 宣言", () => {
  it('$palette.source = "swatch" のフィールドをチップ検出の入力にする', () => {
    ...
    const declared = listImageFields(NTS_WORK).filter(
      (f) => f.paletteSource === "swatch",
    );
  ...
  it('$palette.source = "artwork" のフィールドを透過抽出の入力にする', () => {
    expect(
      resolvePaletteImageFields(NTS_WORK, "artwork").map((f) => f.key),
    ).toEqual(["corefolder_PNGPath"]);
    expect(
      resolvePaletteImageFields(UBL_WORK, "artwork").map((f) => f.key),
  ...
  it("宣言の無い作品ではフォールバックをそのまま返す（既存作品の挙動を変えない）", () => {
    ...
    expect(resolvePaletteImageFields(noDecl, "swatch", fallback)).toEqual(
      fallback,
    );
    expect(resolvePaletteImageFields(null, "swatch", fallback)).toEqual(
      fallback,
    );
  });
...
describe("resolveArtworkSources — 拡張子の有無を吸収する", () => {
  it("拡張子なしのパス（corefolder_PNGPath）を解決する", () => {
    const record = {
      Images: { corefolder_PNGPath: ["1/emstk_corefolderNTS-1-1"] },
    };
    const sources = resolveArtworkSources(record, NTS_WORK, IMAGES);
    expect(sources.map((s) => path.basename(s.path))).toContain(
      "emstk_corefolderNTS-1-1.png",
    );
  });
  ...
  it("拡張子込みのパス（keycapper_PNGPath）でも二重に付けない", () => {
    const record = {
      Images: { keycapper_PNGPath: ["I/emstk_keycapperUBL-Ig2-1.png"] },
    };
    const sources = resolveArtworkSources(
      record,
      UBL_WORK,
      path.join(UBL_WORK, "Images", "DB_Primary"),
    );
...
describe("detectArtworkColorsForRecord — レコード単位の抽出", () => {
  it("透過イラストを持つレコードから配色と出典を返す", () => {
    ...
    const { colors, source } = detectArtworkColorsForRecord(
      record,
      NTS_WORK,
      IMAGES,
      {
...
describe("verifyArtworkAgainstChips — チップ由来パレットとの照合（精度の回帰）", () => {
  it("NumberTales Primary の抽出色が、作者指定のチップ色と高い割合で一致する", () => {
    const { totals } = verifyArtworkAgainstChips({
      work: "NumberTales",
      db: "Primary",
      minRatio: 0.02,
    });
...
describe("applySlotAssignment — 配色スロットの確定（並び順 / Role / 色名）", () => {
  ...
  const NTS1_ASSIGNMENT = [
    ...
    },
    {
      slot: "secondary",
      hex: "#FFAC8F",
      appliesTo: ["#BodyPart_Tail", "#BodyPart_Ear"],
    },
    {
      slot: "accentMain",
      hex: "#E55951",
      appliesTo: ["#BodyPart_Eye", "#BodyPart_Chest"],
    },
    {
      slot: "accentSub",
      hex: "#C9CDCB",
      appliesTo: ["#BodyPart_Eye", "#BodyPart_Foot"],
    },
    {
      slot: "secondaryCostume",
      hex: "#CEC7B6",
      appliesTo: ["#BodyPart_Waist", "#BodyPart_Foot"],
    },
  ...
  it("アクセント枠だけ AppliesTo から部位注記を付ける", () => {
    ...
    expect(eyeOnly).toEqual({
      jp: "メインアクセントカラー（瞳）",
      en: "Main Accent Color (Eye Color)",
    });

    const accessoryOnly = buildSlotColorName(COLOR_SLOTS[4], [
      "#BodyPart_Foot",
    ]);
    expect(accessoryOnly).toEqual({
      jp: "サブアクセントカラー（アクセサリー）",
      en: "Sub Accent Color (Accessory Color)",
    });

    // アクセント以外は注記を付けない
    expect(buildSlotColorName(COLOR_SLOTS[0], ["#BodyPart_Hair"])).toEqual({
      jp: "主色",
      en: "Primary Color",
    });
  });
  ...
  it("手で書き換えられた色名の注記は上書きしない", () => {
    ...
    expect(value[0].ColorName_JP).toBe(
      "メインアクセントカラー（瞳, 衣装補足色）",
    );
    expect(value[0].ColorName_EN).toBe(
      "Main Accent Color (Eye Color, Costume Auxiliary Color)",
    );
  });
  ...
  it("生成形の色名は AppliesTo の変更に追従して書き換わる", () => {
    const palette = [
      {
        Hex: "#009489",
        ColorName_JP: "メインアクセントカラー（アクセサリー）",
      },
    ];
    const { value } = applySlotAssignment(palette, [
  ...
  it("割当に載っていない色は補助色へ流さず unassigned で返す", () => {
    ...
    const { value, unassigned } = applySlotAssignment(palette, [
      { slot: "primary", hex: "#ED5D47" },
    ]);
  ...
  it("未知のスロット名・存在しない Hex は打ち間違いとして弾く", () => {
    expect(() =>
      applySlotAssignment(
        [{ Hex: "#ED5D47" }],
        [{ slot: "tertiary", hex: "#ED5D47" }],
      ),
    ).toThrow(/未知のスロット名/);
    expect(() =>
      applySlotAssignment(
        [{ Hex: "#ED5D47" }],
        [{ slot: "primary", hex: "#000000" }],
      ),
    ).toThrow(/無い Hex/);
  });
});
...
describe("classifyParts — AppliesTo を地毛 / 衣装 / アクセサリーへ写す", () => {
  it("髪・耳・尻尾は地毛", () => {
    expect(classifyParts(["#BodyPart_Hair", "#BodyPart_Tail"])).toEqual({
      base: true,
      costume: false,
      accessory: false,
      empty: false,
  ...
  it("胸・腰・脚は衣装", () => {
    expect(
      classifyParts(["#BodyPart_Chest", "#BodyPart_Waist", "#BodyPart_Leg"]),
...
describe("collectSlotEvidence — 判定材料に AppliesTo を含める", () => {
  ...
  it("Num 1 の各色に、レコードの AppliesTo がそのまま乗る", () => {
    const record = JSON.parse(
      fs.readFileSync(
        path.join(NTS_WORK, "DataBases", "db_Primary.json"),
        "utf8",
      ),
    ).find((r) => r.Num === 1);
    ...
    expect(byHex.get("#ED5D47")).toEqual([
      "#BodyPart_Hair",
      "#BodyPart_Ear",
      "#BodyPart_Tail",
    ]);
    expect(byHex.get("#FF8682")).toEqual([
      "#BodyPart_Chest",
      "#BodyPart_Shoulder",
    ]);
...
describe("findFigureBlobs / measureFigureBands — 設定画から人姿を切り出して帯で読む", () => {
  /**
   * Num 24（フトシ）の設定画。作者が確定させた担当は判っている:
   *   `#E8AFD8` 主色（髪）/ `#AEB8DB` 主色(衣装)（カーディガンとスカート）/
   *   `#FCE8EC` 副色（尻尾の先端）/ `#0097C9` メインアクセント（ネクタイ）/
   *   `#C680AF` 副色（衣装）（裾と靴）
   *
   * このレコードは**被覆率では衣装枠の順序を外す**（人姿イラストの計測で
   * 藤色 `#C680AF` 19.9% > 青灰 `#AEB8DB` 5.8%）。帯で分けられることを確かめる。
   */
  const NUM24_HEXES = ["#E8AFD8", "#AEB8DB", "#FCE8EC", "#0097C9", "#C680AF"];
  const concept = () =>
    decodePng(
      fs.readFileSync(path.join(IMAGES, "concept", "cnsp_imgNTS-24.png")),
    );

  it("注釈文字を落とし、面積の大きい塊だけを返す", () => {
    const blobs = findFigureBlobs(concept());
    expect(blobs.length).toBeGreaterThan(0);
    expect(blobs.length).toBeLessThanOrEqual(3);
    // 面積の降順で、各塊は画像内に収まっていること
    for (let i = 1; i < blobs.length; i++)
      expect(blobs[i - 1].cells).toBeGreaterThanOrEqual(blobs[i].cells);
  ...
  it("頭の帯は髪の色、下の帯は下衣の色が最上位になる", () => {
    const img = concept();
    const [main] = findFigureBlobs(img);
    const bands = measureFigureBands(img, NUM24_HEXES, main.box, {
      common: readCommonColors(NTS_WORK),
    });

    expect(bands).toHaveLength(5);
    expect(bands[0][0].hex).toBe("#E8AFD8"); // 頭 = 髪（主色）
    // 中〜下は衣装。被覆率では 5.8% しか出ない青灰がここでは首位に来る
    expect(bands[2][0].hex).toBe("#AEB8DB"); // 中 = カーディガンとスカート（主色(衣装)）
    expect(bands[3][0].hex).toBe("#C680AF"); // 下 = 裾（副色（衣装））
  });

  it("共通造形色（肌）を除外しないと胴の帯が肌色へ吸われる", () => {
    const img = concept();
    const [main] = findFigureBlobs(img);
    const withCommon = measureFigureBands(img, NUM24_HEXES, main.box, {
      common: readCommonColors(NTS_WORK),
    });
    const without = measureFigureBands(img, NUM24_HEXES, main.box);
    // 除外しないほうが必ず画素数が多くなる（肌が最近傍の配色へ割り当てられるため）
    const sum = (bands) => bands.flat().reduce((n, t) => n + t.share, 0);
    expect(sum(without)).toBeGreaterThan(0);
    expect(withCommon.flat().length).toBeGreaterThan(0);
    // 除外ありでは肌に近い色のシェアが下がる
    const skinNear = (bands, bi) =>
      bands[bi].find((t) => t.hex === "#FCE8EC")?.share ?? 0;
    expect(skinNear(withCommon, 1)).toBeLessThanOrEqual(skinNear(without, 1));
  });
});
...
describe("proposeSlotAssignment — 下書き（主色・副色のみ）", () => {
  ...
  const hair = (hex, ball, art = 0) => ({
    ...
    hints: [
      { word: "red", bodyPart: "#BodyPart_Hair", element: "#Element_Motif" },
    ],
  });
  ...
  it("衣装色・アクセント色は推定しない（未割当のまま返す）", () => {
    const { assignment } = proposeSlotAssignment([
      hair("#111111", 0.6),
      hair("#222222", 0.2),
      costume("#333333", 0.01, 0.4),
      {
        hex: "#444444",
        covBall: 0.01,
        covArt: 0.01,
        bands: [0, 0, 0, 0, 0],
        appliesTo: ["#BodyPart_Eye"],
        hints: [],
      },
    ]);
  ...
  it("球体型姿が無い場合は人姿のシェアで代用する", () => {
    ...
    expect(assignment.map((a) => [a.slot, a.hex])).toEqual([
      ["primary", "#222222"],
      ["secondary", "#111111"],
    ]);
  });
...
describe("profileColorBands / renderColorMap — 近似色を「どこに出ているか」で分ける", () => {
   ...
   */
  const NUM1_COREFOLDER = path.join(
    IMAGES,
    "corefolder",
    "1",
    "emstk_corefolderNTS-1-1.png",
  ...
  it("主色が最大シェアになり、帯の合計は 1 になる", () => {
    ...
    const top = prof.indexOf(
      prof.reduce((a, b) => (b.share > a.share ? b : a)),
    );
...
describe("recordLabel — Num を持たない作品の表示", () => {
  ...
  it("Num が無ければ先頭のインデックス項目を使う（ハンカクライブの Letter）", () => {
    expect(recordLabel({ Letter: { Alphabet: "I", AlphaGen: 2 } })).toBe(
      "Letter:I/2",
    );
  });
});
