...
- **AppearanceDetail系**: `design-part-schema` → `appearance-attrs-typed-schema` → `appearance-detail-merge-integration` / `runbook` → `appearance-detail-cleanup`（`Costume`フィールド新設・BodyPart enum拡張含むP1最終対応）まで実装完了につき、いずれも `.completed/` へ退避済み。develop側で追加の残作業だった NumberTales `NumberMarkLocation`/`IdentityMotif` 廃止も 2026-07-11 に実装完了（`appearance-detail-cleanup` という名前は上記Costume新設タスクと重複するため、こちらのログは `2026-07-11_progress_remove-nummark-identitymotif.md` で別管理）。addon-ai-tag側のAIHints追従対応は本ブランチでこれから着手。
