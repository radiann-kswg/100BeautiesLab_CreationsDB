...
| GitHub Issue/PR トリアージ            | [2026-07-22_github-triage.md](./2026-07-22_github-triage.md)                                                               | 🟡 Issue #13（希望タスク）を新規検知。Cloudflare API同期失敗は既知の一過性として鎮静化。**§3 の AIHints 再同期失敗の原因仮説（`AI_Optout` exit 2）は誤りと判明** → [2026-07-25_progress_aihints-resync-ci-failure.md](./2026-07-25_progress_aihints-resync-ci-failure.md) で訂正済み・提案対応は不要 |
| AIHints 再同期 CI 失敗と復旧          | [2026-07-25_progress_aihints-resync-ci-failure.md](./2026-07-25_progress_aihints-resync-ci-failure.md)                     | 🟢 原因（辞書構造変更へのテスト追従漏れ）を特定し `addon-ai-tag` で修正・本番 Actions で緑を確認（`dde4484` / PR #14）。見送りは actions v4 の Node 20 非推奨警告（リポジトリ横断で別途）                                                                                    |
