...
| GitHub Issue/PR トリアージ               | [2026-07-18_github-triage.md](./2026-07-18_github-triage.md)                                                                                                        | 🟡 新規CI失敗を複数リポジトリで検知（本リポジトリのJekyll/AIHints再同期、CheatSheet-of_HttpResponceDataCode）。詳細は当該ログ参照（過去の日次ログは `.completed/` へ退避） |
...
| **addon-ai-tag**: AIHints 残課題台帳     | [2026-07-14_progress_addon-ai-tag-log-inventory.md](./2026-07-14_progress_addon-ai-tag-log-inventory.md)                                                            | ⚠️ AIHints 系ログ 4 件を退避し、残課題（A1〜A8）を集約（本ブランチ固有。母艦は develop と共有のため衝突回避で分離） |
