...
- 2026-07-04_progress_issue-feature.md（GitHub Issues 機能有効化 + Issue テンプレート追加 + サイト連携（キャラ詳細「データの誤りを報告」リンク）。実装・テスト・目視確認済み。GitHub上でのテンプレート最終表示確認が未完了）
...
- 2026-07-01_progress_addon-ai-tag-revert-cascade-recovery.md（`addon-ai-tag` マージ事故〈逆マージ+revert 巻き込み〉での AIHints 消失リスクに対する調査・復旧ログ。`npm test` 147 pass を確認）
...
- 2026-07-02_progress_data-en-gap-fill.md（`data/` 全体で JP はあるが EN 未指定な箇所を洗い出し、`localize-en-draft` Skill で下書き翻訳。✅ NumberTales db_Primary/ref_Reference 計9件完了。✅ PastDivers `ChronoizedPurity_JP`/`_EN` は翻訳ではなくスキーマの問題と判明し `BustSize` 同様の共有フィールド（`ChronoizedPurity`）へ統合・13件リネーム。`npm test` 152 passed。⚠️ ダイアログ下書き4箇所（NumberTales）は口調確認推奨）
