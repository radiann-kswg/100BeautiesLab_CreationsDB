...
> **⚠️ 以下は 2026-07-13 時点の記載です。第0〜2階はすべて `addon-ai-tag` で実装・適用が完了しました
> （2026-07-25 に実データで確認）。詳細は末尾の「追記（2026-07-25）」を参照してください。**

- ~~本提案の優先度判断・着手順の確定は User 判断待ち。~~ → **完了**
- ~~実装着手時、サブローカル（`addon-ai-tag` チェックアウト先）の物理パスを Claude へ共有する必要がある。~~ → **完了**
- `concept` 画像（設定画）を抽出ソースに含めるかの最終判断（赤ペン注釈のノイズ耐性が確認できてから）。
  → 第2階は **`develop` の `ColorPalette`（設定画のカラーチップ実測）** で達成されたため、本項は事実上解消。
- `AIHints` を持たない 13 件、および画像を持たない 10 件の扱い（対象外とするか、別途 scaffold するか）。
  → **継続**。`addon-ai-tag` の残課題台帳 **A3** で管理。

---

## 追記（2026-07-25）: 3 階すべて完了・`palette_priority` は確定 91 件

> **本ログは `develop` 側のため、`addon-ai-tag` で進んだ実装が反映されていませんでした。**
> 本文の「デッドロック」「92/92 件 `null`」は**着手時点（2026-07-13）の診断**であり、現在の状態ではありません。

| 階 | 内容 | 状態（2026-07-25 実測） |
| --- | --- | --- |
| 第0階 | `palette_priority` の `null` ハンドリング 3 点 | ✅ 実装済み |
| 第1階 | `_meta` provenance + `--resync-structural` + CI ワークフロー | ✅ **稼働済み**（本番 Actions で PR 自動作成 → マージ後 no-op まで確認・PR #14） |
| 第2階 | 決定論的なカラー抽出 | ✅ 当初案（median-cut 推定）ではなく、`develop` の **`ColorPalette`**（設定画のカラーチップ実測）＋ **`--apply-colorpalette`** による機械導出で達成 |

**実データ（NumberTales / Primary）**: `AIHints` 92 件 / **`palette_priority` 確定 91 件** /
`null` 1 件（Num `10-alt` = `ColorPalette` を持たないレコードで、**ソースが無いため null が正しい**）/
`_meta` 92 / 92。`--apply-colorpalette` の dry-run は **`No changes to write.`**（適用済みかつ冪等）。

→ **本ログが提起したデッドロックは完全に解消済み**です。以降は設計背景の参照用として残します。

**注意（再発防止）**: AIHints のコード・スキーマは `develop` に含めない運用のため、
**`develop` 側のログだけを読むと AIHints の実装状況を必ず古く見積もる**。状態を書くときは
`addon-ai-tag` をチェックアウトして実データを見ること。残課題は同ブランチの
