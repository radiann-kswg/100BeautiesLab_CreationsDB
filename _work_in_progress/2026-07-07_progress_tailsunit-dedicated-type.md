...
- `img_PNGName` 相当の参考画像表示は今回もスコープ外（新typedefにも器を持たせていない）。
- `TailsUnit` が基本情報テーブル（一行サマリー）と独立セクション（`tailsUnitSection`、現状未使用）のどちらで表示されるべきかは、複合形状キャラ（2要素以上）での見え方も含めて、Userの実際の好みを見ながら今後調整の余地がある。
- 並行編集の実態（他ローカル/セッションで何が行われているか）をUserに確認いただきたい。
