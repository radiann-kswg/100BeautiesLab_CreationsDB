...
## 追記（2026-09-07）: 差し替え完了 ✅

User が原本を投入し（`e95b720` 画像整備(ナンバーテールズ) その１）、**依頼2 は達成**した。

| 指標 | 差し替え前 | 差し替え後 |
| --- | ---: | ---: |
| corefolder 枚数 | 201 | **202**（収録漏れ 1 枚を追加） |
| 長辺 1024px 未満 | 194 枚 | **0 枚** |
| 長辺 最小 / 中央値 / 最大 | 402 / 477 / — px | **1235 / 1557 / 3481 px** |
| corefolder 合計サイズ | — | **30.5 MB** |
| `data/` 全画像（png+jpg） | 733 枚 / 71.0 MB | 734 枚 / **81.0 MB** |

拡大率は 3.25 倍。上表の見積もり（1536px で約 90 MB）に対して実際は **+10 MB** に収まった。
拡大後も PNG のべた塗り構造がよく保たれた（Num 1 でユニーク色 52 → 121）ためと考えられる。

### 併せて実施したこと

1. **画像参照の全数チェック**: 全作品 21 DB の `Images` 参照 **643 件**を実ファイルと突き合わせ、
   実体の無い参照が **0 件**であることを確認した。
   - 途中で 1 件の破損参照を発見・修正: `db_Primary.json` の `#47` / `#74` が
     `chattingArt/2023/chart_imgNTS-47,NTS-74-humanoid` を指していたが、`chattingArt/` に年フォルダは無い
     （`humanoids/` は年フォルダ運用のため、そちらからのコピペ由来と見られる。`cc0aa87` から存在）。
     `2023/` を除いて解決。**本 2 レコードの絵チャット画像は、それまで表示できていなかった**。

2. **`patch-colorpalette` のテスト 2 件の追従**（下記）。

### 拡大処理による色の一律 -1 シフト（テスト 2 件の失敗原因）

差し替え後 `tests/patch-colorpalette.test.js` の 2 件が失敗していた。原因は
**拡大処理の色空間往復で全チャンネルが一律 -1 ずれた**こと。
...
**実装側は無傷**であることを実測で確認済み。

- 共通造形色の除外は正常動作（`#FEFEFD` は `SOLID_EXCLUDE_TOL = 6` に対し距離 2.45 で除外される）
- 抽出値とチップ由来パレットの距離は **1.00〜1.73**（チップ同士の最小距離 30.6 に対して十分小さい）
- 色数・面積比の順序・マージ挙動はいずれも保たれている

したがって `AGENTS.md`「データ更新時のテスト追従」に従い、**テスト側を新しいデータ仕様へ追従**させた。
完全一致（`toContain`）から**色差による判定**（閾値 3）へ変更し、なぜ完全一致が成立しなくなったかを
テスト内に注釈として残している。閾値 3 は符号化差（最大 1.73）を通し、別の塗りとの取り違え（最小 30.6）は
通さない水準。

### ⚠️ 発見した別件: `measurePaletteCoverage` の主色判定が変わった

差し替えに伴い、**Role（Primary/Secondary/Accent）の根拠である被覆率の測定結果が大きく変わった**。
テストは通っている（既存テストが「合計 ≤ 1」「> 0」しか見ていないため）が、実害があり得るので記録する。

Num 1 の実測（チップ 4 色に対する被覆率）:

| | `#ED5D47` | `#FF8682` | `#FFBFA7` | `#FFAC8F` |
| --- | ---: | ---: | ---: | ---: |
| 差し替え前 | **71.9%** | 0.5% | 21.8% | 0.0% |
| 差し替え後 | 6.4% | **54.9%** | 19.6% | 16.5% |
| 生ヒストグラム（真値） | **30.6%** | 25.2% | 9.3% | 7.7% |

原因は `measurePaletteCoverage()` が使う `buildForegroundMask()`。同関数は
**外周からのフラッドフィルと外周の色分布による背景色推定**を行うが、これは紙面付きの設定画を
想定したもので、透過素材では外周に何が写り込むかで結果が振れる。実際、外周の不透明画素は
差し替え前が「文字色混じり（`#ED5D47`:152 / `#FFAC8F`:75 / `#FFFFFF`:69）」、差し替え後は
「ほぼ黒の輪郭線のみ」へ変わっていた。前後とも不透明画素の **40〜43% しか前景として残っていない**。

> `extractSolidColors()` の JSDoc は「`buildForegroundMask()` も使わない。…キャラクターの白い塗りを
> 紙面と誤判定して落としてしまう」と明記しており、**この脆さは既知**。今回それが被覆率側で表面化した。

**本ラウンドでは修正していない。** 透過素材向けに「不透明かつ輪郭線の純黒でない画素」を前景とする
代案を試作したところ、監修済み `ColorPalette` の先頭色との一致は 90 件中 65 → 77 件（72% → 86%）へ
改善する一方、**白毛のキャラで白を主色と誤判定する新しい失敗モード**が出た（`#FFEFE4` / `#F4FAE8` 等 8 件）。
加えて「監修済みパレットの先頭 = 面積最大」という判定基準自体が正しいとは限らない
（User が入れた先頭色は概念上の主色であり、面積最大とは限らない）。
**色の主従は創作判断に踏み込む領域**のため、User の判断を仰ぐ形で残す（母艦 **T-37**）。
...
> **注記（2026-09-07）**: 以下は**差し替え前**のスナップショットです。作業リストとしての役目は終えており、
> 現在は全 202 枚が 1024px 以上（対象 0 枚）。当時どのファイルがどの解像度だったかの記録として残します。
