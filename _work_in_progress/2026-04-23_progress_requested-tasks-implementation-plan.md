...
- タスク 4 の最小案として `isPrivate: true` を公開除外フラグに採用し、SW の `db` / `search` 応答から該当レコードを外す実装に着手した。
- enrich の参照経路でも private レコードを検索対象にしないよう `lib/data-common.js` を合わせて更新した。
- UI 側は通常経路では filtered records だけを扱い、private レコードが直接詳細表示へ渡された場合も本文を描画しないガードを追加した。
