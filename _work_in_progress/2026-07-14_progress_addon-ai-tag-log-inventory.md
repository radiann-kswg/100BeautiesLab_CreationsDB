...
## 追記（2026-08-20）: 逆マージ事故ログの後日談（T-31）

- `develop` 側で保留として残していた「`addon-ai-tag` 側 2026-07-01 ログへの後日談追記」を実施するため、
  本ブランチで実体確認を行った。
- 結果として、対象とされていた
  `_work_in_progress/2026-07-01_progress_addon-ai-tag-merge-conflict-and-log-cleanup.md` は
  `origin/addon-ai-tag` に存在しないことを確認した。
- そのため、後日談の追記先は本ログ（`2026-07-14_progress_addon-ai-tag-log-inventory.md`）を正として統合し、
  逆マージ事故ログ（`2026-07-02_progress_addon-ai-tag-reverse-merge-incident.md`）および
  母艦台帳（`2026-07-25_remaining-task.md`）へ状態を同期した。

### 補足

- 本追記により T-31 の「addon-ai-tag 側での後日談追記」は完了扱いとする。
- 以後、逆マージ事故関連の参照先は `2026-07-02`（事故経緯）と本ログ `2026-07-14`（addon 側棚卸し）を正とする。
