...
### 完了（追記 20・開発環境の動作確認と棚卸 / 2026-08-09）

User 指示「開発環境の動作確認を済ませ、進捗ログ更新と棚卸を行う」に対応。

- **ブラウザ実動作確認（http://127.0.0.1:5500）**
  - 相関図ページ（`pages/relations.html?d=%23Works_UnibyteLive&lang=jp`）で表示を確認。
  - グルーピング `なし → クラス名` の切替で URL が `g=Class` へ更新されることを確認。
  - 言語トグルで URL が `lang=jp → lang=en` へ更新され、ボタンラベル群が英語化されることを確認。
  - 統計表示（例: `表示: 6 ノード / 5 本（マス塗り）`）が継続して表示されることを確認。
  - `index.html` の `相関図UI` 導線、`pages/characters.html` の `相関図を開く` 導線が実遷移することを確認。

- **テスト確認**
  - 既実行のフルテスト結果を再確認: `67 files / 1181 tests` 成功。

- **観測メモ（非ブロッカー）**
  - 相関図遷移直後にブラウザコンソールで 404 が 1 件出たが、直後に相関図のデータ読込・表示は正常化（`Service Worker 準備中` → `データを読み込み中` → 通常表示）し、再現する機能不全は確認できず。

- **棚卸結果**
  - 本件（相関図新設）の残作業だった `Phase 3-D / 3-E / 4` は完了。
  - 残タスク台帳（`2026-07-25_remaining-task.md`）側の `T-13` は完了へ更新。
