...
```.Markdown
`./data/Works_NumberTales/DataBases/db_Primary.json`の`ThirdPersonCalling`を参考に、全データベースにあるキャラクターの呼称方法に該当する`FirstPersonCalling`,`SecondPersonCalling`,`ThirdPersonCalling`フィールドのデータを以下の書式法則に従うようデータを成型してください。ただしデータの変更は必ず、もともとあったキャラクター設定として内容や意味が一切変わらない範囲でお願いします。
なお データベースの直接操作となるため、データ表記上で不明瞭な点などは独断で変更せずuserに確認するようにしてください。
### データ書式
- 「`{性別呼び};{こそあど呼び};{名前呼び};{形容詞呼び}`」の記法に則った１つのデータ節をベースに扱う
- 個々のデータ節内は、性別や対象ごとに`/`,特に意味のない使い分けごとに`,`で区切られている
- キャラクターのTPOごとにデータ節を複数入力するときは`\n`で区切る（データ節の概要を記載する際はデータ節の冒頭に区切り文字`：`を挟んで追記してもよい、例：`●●の場合：`）
- `[※名前呼び]`,`[※二人称]`などは呼称する相手キャラクターの名前や別の呼称方法を参照するときに記載する
- 要所要所に適宜`※`を冒頭文字に注釈を入れてもよい（例：`※稀` など）
- こそあど言葉のワイルドカード文字は`＊`、名称などのワイルドカード文字は`~`を使用する（例：`＊れ`(これ,それ,あれ,どれ), `~さん`(さん付け)）
- 平仮名と片仮名と漢字は別の表現として扱う
```
