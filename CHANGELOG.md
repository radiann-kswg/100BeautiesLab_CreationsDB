...
- **`proposeSlotAssignment()` の副色を地毛で絞った**: 確定 5 件（Num 1/5/6/8/24）での正解率が **90% → 100%**（主色 5/5・副色 5/5）。旧版は Num 1 のパーカー `#FF8682`（衣装 33.2%）が耳・尻尾の `#FFAC8F`（11.4%）を上回るため副色を外していた。**主色は絞らない**（`AppliesTo` の抜けに巻き込まれる。Num 9 の `#A1A9BF` は球体型姿の 71.8% を占める髪だが `AppliesTo` が空）。
- **下書きが `appliesTo` を出さなくなった**: 色語から組み立てた部位を書くと、User が「侵食されている」として差し戻した経路が下書き経由で復活する。キーを省けば `applySlotAssignment()` が既存値を持ち越す。
- **衣装枠の順序は自動化しない（実測による判断）**: 主色(衣装) と 副色（衣装）の別はシェアでは決まらない（確定 5 件で 3/5）。Num 24 は青灰 `#AEB8DB` のカーディガンが主だが、人姿イラストの計測では藤色 `#C680AF`（19.9%）が青灰（5.8%）を上回る。**最終判断は設定画**とし、ツールは根拠提示に徹する。
...
- **`Beast_Badge`（ShouArRiders）は辞書参照**: `dict_Beast.json` に `Beast_Badge` 列（`EZ1`〜`EZ13` の全 15 値。戌年が Raccoon / Fox / Dog の 3 行に分かれるため衝突回避の `EZ11A`(Fox) / `EZ11B`(Raccoon) を含む）を持ち、`$badge` を `{ "keys": [{ "key": "Beast", "codeFrom": "Beast_Badge" }] }` として**レコードへ複製せず辞書行から解決**する。`$dictRef` と同じ「値は辞書で一元管理する」方針。
...
- **`AGENTS.md`「サブローカル並行作業運用（予備作業場）」節**: 環境構成の記述を実態へ更新（サブローカル ×2 → **×3**）。4 環境の呼称を **`main` / `sub1` / `sub2` / `sub3`** と定義し、現在の担当は `README.LOCAL.md` に記載する旨を明記。「特定の作業内容を恒久割り当てしない」原則に対し、`## 作業分担` 節で暫定の担当分けが定義されている場合はそれを優先する例外を追加した。
- **`AGENTS.md`「`README.LOCAL.md`」節にサブ節「ローカル環境の作業分担（`## 作業分担` 節）」を新設**: (1) 着手前に `## 作業分担` / `### この環境の担当` を読む義務（担当外の作業は着手前に User へ確認）、(2) 節の位置は `## 定時議事録` より前、(3) 書式（見出し＋状況＋4 環境の分担表＋運用注記＋`### この環境の担当`）、(4) 分担表は全ローカルで同一に保ち、変更時は 4 環境すべてを同じ更新日で書き換える同期ルール、(5) 分担の作成・変更は User の指示があったときのみ、(6) 節が無い／古い場合は「担当未定」として User に確認、(7) 代行時は双方の `### この環境の担当` に引継ぎを明記、(8) 自動議事録タスク `local-readme-minutes` は `## 作業分担` 節を書き換えない、(9) `_work_in_progress/` ・`## 定時議事録` との役割分担、を規定した。
- **背景**: `README.LOCAL.md` は `.gitignore` 対象で commit / pull による自動配布が効かない。そのため「ルール（配布される）」を `AGENTS.md`、「分担内容（配布されない）」を各 `README.LOCAL.md` に置き、変更時は全環境を手動で揃える運用とした。
- **影響範囲**: `AGENTS.md` / 生成物 `.github/copilot-instructions.md`（`npm run agents:build`）。
- **検証**: `npm run agents:check`（生成物は正典と一致）/ `npm test`。
