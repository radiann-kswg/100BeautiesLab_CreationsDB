...
(function initializeCharacterSectionRendererRegistry(root) {
	...
	function renderBuiltInRelationSection(item, context = {}) {
		...
		const renderRelationTag = (prefix, relationRecord, withLabels) => {
			...
			// 言語フラグに基づいて Comments/Comments_EN を切り替える
	...
	// IdentityMotif (#Def_FormsMotif[]) の Formation+Motif 配列構造を担当する built-in renderer
	registerSectionRenderer('formsMotifSection', {
		match: (context) => (
			Array.isArray(context?.item?.value)
			&& String(context?.item?.type || '').includes('#Def_FormsMotif')
			&& typeof context?.helpers?.renderFormsMotifSection === 'function'
		),
		render: (item, context) => {
			if (typeof context?.helpers?.renderFormsMotifSection !== 'function') return null;
			return context.helpers.renderFormsMotifSection(item, context);
		}
	});

	// ThisMasters ($Def_ThisMastersEntry[]) の value_JP/about_JP 配列構造を担当する built-in renderer
	registerSectionRenderer('thisMastersSection', {
		match: (context) => (
			Array.isArray(context?.item?.value)
			&& String(context?.item?.type || '').includes('$Def_ThisMastersEntry')
			&& typeof context?.helpers?.renderThisMastersSection === 'function'
		),
		render: (item, context) => {
			if (typeof context?.helpers?.renderThisMastersSection !== 'function') return null;
			return context.helpers.renderThisMastersSection(item, context);
		}
	});
