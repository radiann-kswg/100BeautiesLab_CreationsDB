    ...
    try {
      ...
      return records.map(rec => {
        ...
        for (const [k, v] of Object.entries(defaults)) {
          ...
          if (typeof rec[k] === 'undefined') rec[k] = v; // レコード入力が最優先
        }
        return rec;
      });
    } catch (_) {
...
class StandardEndpointHandlers {
  ...
  async handleSearchEndpoint(url, resolve, debug, enrich = false) {
    ...
    let matched = SearchEngine.searchRecords(records, queries);
    ...
    // 充実化処理
    if (enrich && this.enrichmentProcessor) {
      matched = await this.enrichmentProcessor.enrichRecords(matched, workId);
    }
