...
class DataFetcher {
  ...
  async readGlobalMeta() {
    return this.fetchJSON('/data/db_meta.json');
  }
  ...
  async readWorkMeta(workId) {
    ...
    return this.fetchJSON(metaPath);
  }
  ...
  async readWorkType(workId) {
    if (!workId) throw new Error('Invalid workId');
    try {
      return await this.fetchJSON(`/data/${workId.replace('#Works_', 'Works_')}/DataBases/db_type.json`);
    } catch (_) {
      return {};
    }
  }
  ...
  async listWorkDBs(workId) {
    ...
    if (exist.length === 0) {
      const conventional = [
        ...
        { name: 'UnproceededSecondary', file: 'db_UnproceededSecondary.json' },
...
class ApiEndpointHandlers {
  ...
  async handleDeftypeGlobalEndpoint() {
    // NOTE: 「deftype」は、typedef(db_type.json) ではなく、
    // 表示名解決に使う定義辞書(db_meta.json の $VarsDef: $EnumDef_*, #List_* など) を返す。
    // UI 側（pages/characters.js）はこのレスポンスを参照して GenderType / RaceType 等をJP化する。
    const globalMeta = await this.dataFetcher.readGlobalMeta();
    return ResponseUtils.jsonResponse(globalMeta || {});
  }
...
class StandardEndpointHandlers {
  ...
  async handleIndexEndpoint() {
    ...
    const response = {
      ...
      version: 1,
      time: new Date().toISOString(),
      works: Object.entries(globalMeta.CreationWorks).map(([k, v]) => ({
        key: k,
        Title: v.Title,
        Title_EN: v.Title_EN
      }))
  ...
  async handleWorksListEndpoint() {
    const globalMeta = await this.dataFetcher.readGlobalMeta();
    const works = Object.entries(globalMeta.CreationWorks).map(([k, v]) => ({
      key: k,
      Title: v.Title,
      Title_EN: v.Title_EN
    }));
  ...
  async handleBootstrapEndpoint(url, defaultIncludeRecords = false, defaultEnrich = false) {
    ...
    for (const workId of works) {
      const databases = await this.dataFetcher.listWorkDBs(workId);
      const item = { work: workId, databases };

      // 作品メタは存在しない可能性があるため、可能なら読み込む（失敗したら null）
      let workMeta = null;
      try {
  ...
  async handleWorkEndpoint(workIdParam) {
    ...
    try {
      const workMeta = await this.dataFetcher.readWorkMeta(workId);
      return ResponseUtils.jsonResponse({ work: workId, meta: workMeta });
    } catch {
