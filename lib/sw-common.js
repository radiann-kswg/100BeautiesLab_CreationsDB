...
class DataFetcher {
  ...
  resolveDbFilePrefix(metaKey) {
    return String(metaKey || '').startsWith('#Ref_') ? 'ref_' : 'db_';
  }
  ...
  async readWorkMeta(workId) {
    ...
    return this.mergeMetaWithDictionaryBundle(mergedWithRef, dictBundle.vars, dictBundle.meta);
  }
  ...
  async readRefMeta(workId) {
    try {
      return await this.fetchJSON(`/data/${resolveWorkDirName(workId)}/References/db_meta.json`);
    } catch (_) {
      return {};
    }
  }
  ...
  mergeRefDatabases(baseMeta, refMeta) {
    const refDbs = refMeta?.Databases;
    if (!refDbs || typeof refDbs !== 'object' || Array.isArray(refDbs)) return baseMeta;

    const normalized = Object.fromEntries(
      Object.entries(refDbs)
        .filter(([, v]) => v && typeof v === 'object' && !Array.isArray(v))
        .map(([key, entry]) => [
          key,
          typeof entry.DB_Layer === 'string' && entry.DB_Layer.trim()
            ? entry
            : { DB_Layer: 'References', ...entry }
        ])
    );

    const baseDbs = baseMeta?.Databases ?? {};
    // DataBases 側に同一キーがない References エントリのみ末尾に追加（DataBases 先行）
    const refOnlyEntries = Object.fromEntries(
  ...
  async readDB(workId, dbName) {
    ...
    const base = `/data/${resolveWorkDirName(workId)}/${this.resolveDbLayer(dbMeta)}`;
...
class ApiEndpointHandlers {
  ...
  mergeMetaAndTypeVars(metaSource, typeSource) {
    ...
    if (Object.keys(typeVars).length === 0 && !typeMetaType) return meta;
...
class StandardEndpointHandlers {
  ...
  async handleBootstrapEndpoint(url, defaultIncludeRecords = false, defaultEnrich = false) {
    ...
    for (const workId of works) {
      ...
      if (workMeta) {
        ...
        databases = this.decorateDatabaseCatalogEntries(databases, workMeta, [globalType]);
  ...
  async handleWorkDbListEndpoint(workIdParam) {
    ...
    let databases = await this.dataFetcher.listWorkDBs(workId);
    try {
      const [workMeta, globalType] = await Promise.all([
        this.dataFetcher.readWorkMeta(workId),
