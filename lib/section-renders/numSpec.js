/**
 * NumerospecStats 専用セクションレンダラー (numSpecSection)。
 * EffectStats / SafetyLevel を共通ヘルパーで描画し、残りを buildObjectChildBlocks へ委譲する。
 */
(() => {
	const registry = globalThis.CharacterSectionRendererRegistry;
	if (!registry?.registerSectionRenderer) return;

	registry.registerSectionRenderer('numSpecSection', {
