		...
		const hydrateCharacterName = (anchorEl, namePlaceholderEl, idxValue, keyPath, wId, dbName) => {
			fetchCrossDbRecords(wId, dbName).then((records) => {
				if (!Array.isArray(records)) {
					console.warn('[relation.js] hydrateCharacterName: records は配列ではありません', records);
					return;
				}
				...
				if (!found) {
					...
					if (fallback) {
						const name = fallback?.Name || fallback?.FormalName || fallback?.ModelName || fallback?.Name_EN || '';
						...
						console.warn('[relation.js] hydrateCharacterName: 対象キャラが見つかりません', { idxValue, keyPath, dbName, recordCount: records.length });
					}
					return;
				}
				const name = found?.Name || found?.FormalName || found?.ModelName || found?.Name_EN || '';
				if (!name) {
					console.warn('[relation.js] hydrateCharacterName: 名前フィールドが空です', { found: Object.keys(found) });
					return;
				}
				...
				console.warn('[relation.js] hydrateCharacterName: フェッチエラー', err);
			});
		};
