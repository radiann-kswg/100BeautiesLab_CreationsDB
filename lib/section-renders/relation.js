		...
		const hydrateCharacterName = (anchorEl, namePlaceholderEl, idxValue, keyPath, wId, dbName) => {
			fetchCrossDbRecords(wId, dbName).then((records) => {
				...
				if (!found) {
					...
					if (fallback) {
						const name = _hnLang === 'en'
							? (fallback?.Name_EN || fallback?.FormalName_EN || fallback?.ModelName_EN || fallback?.Name_JP || fallback?.FormalName_JP || fallback?.ModelName_JP || '')
							: (fallback?.Name_JP || fallback?.FormalName_JP || fallback?.ModelName_JP || fallback?.Name_EN || '');
