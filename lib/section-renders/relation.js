		...
		const hydrateCharacterName = (anchorEl, namePlaceholderEl, idxValue, keyPath, wId, dbName) => {
			fetchCrossDbRecords(wId, dbName).then((records) => {
				...
				if (!found) {
					...
					if (fallback) {
						const name = fallback?.Name_JP || fallback?.FormalName_JP || fallback?.ModelName_JP || fallback?.Name_EN || '';
				...
				const name = found?.Name_JP || found?.FormalName_JP || found?.ModelName_JP || found?.Name_EN || '';
				if (!name) return;
