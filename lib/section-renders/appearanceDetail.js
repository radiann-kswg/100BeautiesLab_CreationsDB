...
(() => {
	...
	registry.registerSectionRenderer('appearanceDetailSection', {
		...
		render: (item, context) => {
			...
			// --- $EnumDef_* 直参照ルックアップ ---
			// workMeta.$VarsDef は浅いマージのため、グローバル定義が作品定義に上書きされる場合がある。
			// globalDefType（data/db_meta.json + data/db_type.json の合流済み全体定義）をフォールバックとして使用する。
			const varsDef = metaForLookup?.General?.$VarsDef || {};
			const globalVarsDef = globalDefType?.General?.$VarsDef || {};

			/**
			 * `$EnumDef_*` を直接参照してラベルを解決する。
			 * workMeta.$VarsDef を優先し、見つからない場合は globalDefType.$VarsDef にフォールバックする。
			...
			function resolveFromEnumDef(rawValue, enumDefKey, fieldBase) {
				if (!rawValue) return '';
				const enumDef = varsDef[enumDefKey] || globalVarsDef[enumDefKey];
			 ...
			 * - vdict_{DictName}: `$EnumDef_{DictName}` から表示ラベルを解決
			 * - value_Num_1 + value_Num_2: "N × M" 形式でペア表示（Branch 等）
			 * - value_Num: 数値表示（Count / Segment 等）
			 * - value_JP / value_EN: テキスト値（vdict と重複する場合はスキップ）
			...
			function buildAttrRows(attrs) {
				...
				return attrs.flatMap((attr) => {
					...
					// value_JP/EN: vdict と同一内容の場合はスキップして重複を防ぐ
					const textVal = lang === 'en' ? (textEN || textJP) : (textJP || textEN);
					if (textVal && !vdictLabels.includes(textVal)) parts.push(textVal);
