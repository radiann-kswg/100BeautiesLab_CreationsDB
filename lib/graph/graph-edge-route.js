...
export function routeEdges(edges, positions, options = {}) {
	...
	for (const r of routed) {
		...
		// **`segment-weights` が始点/終点ちょうど（0 / 1）に近すぎると、Cytoscape が
		// 「ノードと重なる無効な形状」と誤認して辺を一切描かないバグがある**
		// （`round-segments`/`segments` 双方の実機検証で確認済み。`curve-style: straight` に
		// 戻すと描ける＝スタイル解釈側の問題で、ここで計算しているジオメトリ自体は正しい）。
		// ノード半径ぶんは各脚の向きに沿って内側へ逃がすことで、渡り〜折れ点間の傾き
		// （格子の 6 方向）を厳密に保ったまま、この誤判定を避ける。
