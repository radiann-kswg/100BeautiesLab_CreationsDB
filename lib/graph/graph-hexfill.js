...
import { hexPoint, nearestCell, hexNeighbors, hexDistance, spiralCells } from './graph-layout.js';
 ...
 */
export function relaxSeeds(sizes, iterations = RELAX_ITERATIONS, separation = SEED_SEPARATION_LADDER[0], links = null) {
	...
	return pts.map(p => nearestCell(p.x, p.y, 1));
}
...
function assignHexCellsAt(groups, separation, options = {}) {
	...
	const seeds = relaxSeeds(sizes, RELAX_ITERATIONS, separation, options.links);
	...
	for (let g = 0; g < list.length; g += 1) {
		...
		let seed = null;
		for (const c of spiralCells(seeds[g].col, seeds[g].row, maxRing)) {
		...
		byGroup[g].push(seed);
		remaining[g] -= 1;
		for (const nb of hexNeighbors(seed.col, seed.row)) {
			heaps[g].push({
				cost: 1 + originWeight * hexDistance(nb, origin),
	...
	for (;;) {
		...
		for (let g = 0; g < list.length; g += 1) {
			...
			for (const nb of hexNeighbors(placed.col, placed.row)) {
				if (cells.has(cellKey(nb))) continue;
				heaps[g].push({
					cost: hexDistance(nb, seed) + originWeight * hexDistance(nb, origin),
					group: g, col: nb.col, row: nb.row, from: g
				});
			}
		}
 ...
 * 各グループの境界セルに印を付ける
 *
 * @description 6 近傍のいずれかが「別グループ」または「未割当」なら境界。
 ...
 */
export function markBoundaryCells(cells, byGroup) {
 ...
 */
export function pickAnchorCells(byGroup, spacing = 1) {
	return byGroup.map(list => {
		if (!list || list.length === 0) return null;
		let sx = 0;
		let sy = 0;
		for (const c of list) {
			const p = hexPoint(c.col, c.row, spacing);
			sx += p.x; sy += p.y;
		}
		...
		const p = hexPoint(best.col, best.row, spacing);
 ...
 * グループ同士の隣接関係を求める
 *
 * @description 一方のセルが他方のセルの 6 近傍にあれば隣接。濃度段の貪欲彩色に使う。
 * @param {Map<string, number>} cells
 * @param {Array<Array<{col: number, row: number}>>} byGroup
 * @returns {Array<Set<number>>} グループ番号 -> 隣接グループ番号の集合
 */
export function buildGroupAdjacency(cells, byGroup) {
...
export function buildHexFill(groups, options = {}) {
	...
	const list = Array.isArray(groups) ? groups : [];
	const { cells, byGroup, seeds } = assignHexCells(list, { originWeight: options.originWeight, links: options.links });
	const boundary = markBoundaryCells(cells, byGroup);
	const anchors = pickAnchorCells(byGroup, spacing);
	const shades = assignShadeSteps(buildGroupAdjacency(cells, byGroup), shadeCount);
	...
	// セル半径ぶんだけ外へ広げる（頂点間の高さは spacing × 2/√3）
	const padX = spacing / 2;
	const padY = spacing / Math.sqrt(3);
