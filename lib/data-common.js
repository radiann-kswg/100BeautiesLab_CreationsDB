...
function dbLinkSubsetMatch(recordVal, idxRaw) {
  ...
  return Object.keys(idxRaw).every(k => {
    ...
    if (typeof qv === 'object' && qv !== null) return dbLinkSubsetMatch(rv, qv);
...
class EnrichmentProcessor {
  ...
  async enrichRecords(records, workId, dbName = '') {
    ...
    try {
      ...
      const detailSubFieldSet = Array.isArray(rawSubFields) && rawSubFields.length > 0
        ? new Set(rawSubFields.map(k => String(k).trim()).filter(Boolean))
        : null;
  ...
  async searchRecords(records, workId, dbName, queries) {
    ...
    const valueEquals = (val, qKey, typeSpec) => {
      ...
      // - 例: { hashTag: 'Card', key: { Stoat: 'Major', StoatNum: 0 } } で
      //   Card.Stoat と Card.StoatNum を両方チェックする
