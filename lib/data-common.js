...
/**
 * `Foo_JP` / `Foo_EN` から言語サフィックスを落とす（無ければそのまま）。
 * typedef は `FirstPersonCalling`、実データは `FirstPersonCalling_JP` のように
 * 宣言形式が揃わないため、照合側で差を吸収する。
 * @param {string} key
 * @returns {string}
 */
function stripLangSuffix(key) {
	return String(key || '').replace(/_(JP|EN)$/, '');
}
...
class EnrichmentProcessor {
	 ...
	 * @param {Object} [options] - `declaredKeys`（typedef 宣言）/ `allowKeys`（meta 宣言）等の許可リスト
	...
	mergeFromLinkedRecord(base, linked) {
		...
		// 現在作品の `db_meta.json($DetailLayout)` 由来の許可リスト。指定があれば含まれるキーのみマージ
		const allowKeys = opt.allowKeys instanceof Set ? opt.allowKeys : null;
		...
		for (const [k, v] of Object.entries(linked)) {
			...
			// typedef が suffix 無しで宣言されているケース（`FirstPersonCalling`）に備えてベース名へフォールバックする
			const fieldEntry = fieldEntriesByKey?.get(k) ?? fieldEntriesByKey?.get(stripLangSuffix(k));
			if (!TypeDefUtils.isFieldForSecondaryContext(fieldEntry, isSecondaryContext)) continue;

			// typedef が `$enrich: false` を宣言したフィールドは参照先から埋めない（enrich 禁止宣言）
			if (fieldEntry?.$enrich === false) continue;
			...
			// meta（`$DetailLayout`）に列挙されていない項目は持ち込まない。
			// typedef 宣言だけでは「グローバル宣言の全項目」が通ってしまうため、
			// 参照元作品の meta が表示すると宣言した項目だけに絞る。
	...
	async enrichRecords(records, workId, dbName = '') {
		...
		try {
			...
			// 宣言キーをベース名・_JP・_EN の三方向へ展開して積む（宣言形式とデータのキー形式の差を吸収）
			const addKeysWithLangAliases = (set, list) => {
				if (!Array.isArray(list)) return set;
				for (const raw of list) {
					const k = String(raw || '').trim();
					if (!k) continue;
					const base = stripLangSuffix(k);
					set.add(k);
					if (base !== k) set.add(base);
					else { set.add(`${k}_JP`); set.add(`${k}_EN`); }
				}
				return set;
			};
			const declaredTopLevelKeys = addKeysWithLangAliases(new Set(), typeEntries.map(e => e?.hashTag));
			...
			// db_meta.json $DetailLayout — 宣言済みフィールドの参照解決・マージを制限するために使用
			const detailLayout = ctx?.globalMeta?.CreationWorks?.[workId]?.$DetailLayout;
			// subFields 未登録フィールドの $enrich スキップ／ルート _DBLink の cross-work マージ制限に使用
			const detailSubFieldSet = detailLayout?.subFields?.length
				? addKeysWithLangAliases(new Set(), detailLayout.subFields)
				: null;
			// cross-work マージの許可リスト。typedef 宣言だけではグローバル宣言の全項目が通ってしまうため、
			// 「参照元作品の meta が表示すると宣言した項目」だけに絞る（$alt の代替キーも同じ枠で許可する）
			const detailLayoutFieldSet = (() => {
				if (!detailLayout || typeof detailLayout !== 'object') return null;
				const s = new Set();
				for (const list of [detailLayout.headerPills, detailLayout.basicFields, detailLayout.subFields]) {
					addKeysWithLangAliases(s, list);
				}
				if (s.size === 0) return null;
				for (const [primaryKey, altKeys] of (altFallbackMap ?? [])) {
					if (s.has(primaryKey)) addKeysWithLangAliases(s, altKeys);
				}
				return s;
			})();
			...
			const enrichedRecords = await Promise.all(records.map(async (record) => {
				...
				if (dbLinks.length > 0) {
					...
					if (primaryLinked) {
						...
						const allowKeys = (isCrossWork && detailSubFieldSet) ? detailSubFieldSet : null;
				...
				for (const ent of enrichDbLinkTypes) {
					...
					for (const ref of refs) {
						...
						if (resolved) {
							...
							const isCrossWork = refWorkId !== workId;
							...
							enrichedRecord = this.mergeFromLinkedRecord(enrichedRecord, resolved, {
								allowImages: false,
								indices: ctx?.indices || null,
								declaredKeys: isCrossWork && declaredTopLevelKeys.size > 0 ? declaredTopLevelKeys : null,
								// meta（$DetailLayout）未宣言の項目を参照先から持ち込ませない
								allowKeys: isCrossWork ? detailLayoutFieldSet : null,
								respectExplicitOwnProps: isCrossWork,
								fieldEntriesByKey,
								isSecondaryContext: isCrossWork ? isSecondaryContext : null
							});
							break;
						}
					}
