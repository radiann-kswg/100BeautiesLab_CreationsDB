...
class EnrichmentProcessor {
  ...
  async getWorkContext(workId) {
    ...
    const indexDef = globalMeta?.CreationWorks?.[workId]?.$DefType_Index ?? null;
    ...
    const p = (async () => {
      try {
        ...
        const allRecords = typeof readDB === 'function'
          ...
          : await this.dataFetcher?.fetchJSON?.(`/data/${workKey.replace('#Works_', 'Works_')}/DataBases/db_${dbName}.json`);
      ...
      const enrichedRecords = await Promise.all(records.map(async (record) => {
        ...
        if (dbLinks.length > 0) {
          ...
          if (primaryLinked) {
            ...
            enrichedRecord = this.mergeFromLinkedRecord(enrichedRecord, primaryLinked, { allowImages, indices: ctx?.indices || null });
          }
        }
  ...
  normalizeRecordByTypeDef(record, defTypeMerged) {
    ...
    for (const entry of entries) {
      ...
      const typeSpec = entry?.$type;
      out[key] = TypeDefUtils.normalizeValueByTypeSpec(out[key], typeSpec, { indexDef });
    }
    return out;
  }
  ...
  async searchRecords(records, workId, dbName, queries) {
    ...
    // work の $DefType_Index（data/db_meta.json）を、#Index/$Index の解釈に利用
...
class TypeDefUtils {
  ...
  static looksSearchableType(typeSpec) {
    if (!typeSpec) return false;
    if (typeof typeSpec === 'string') {
      return /(#String|#Summary|#Enum|\$EnumDef)/i.test(typeSpec);
    }
  ...
  static looksIndexType(typeSpec) {
    ...
    return /(^|\||,)\s*(#Index|\$Index)\s*($|\||,)/i.test(typeSpec) || /(\b#Index\b|\b\$Index\b)/i.test(typeSpec);
  }

  /**
   * data/db_meta.json の $DefType_Index を解析して扱いやすい形にする
   * @param {any} indexDef
  ...
  static normalizeValueByTypeSpec(value, typeSpec) {
    ...
    if (typeof typeSpec === 'string' && /(#String|#Summary)/i.test(typeSpec)) {
    ...
    // #Index/$Index は、作品の $DefType_Index がネスト型の場合のみ、最低限の形を補正
