...
/**
 * `_DBCrossLinkPath` 解決専用の絶対パス構築。
 * 既存 ImageProcessor.resolveImagePath() は値にスラッシュを含む場合 folderHint を付与しない
 * 既知の挙動（Images/ セグメントが欠落するケースがある）があるが、`_DBCrossLinkPath` は
 * folderHint を必ず一度だけ含める形でゼロから構築し、この挙動を踏襲しない。
 ...
 * @param {string|null} folderHint
 * @param {string} rawValue - 参照先レコードの画像フィールド生値（相対パス/ファイル名）
...
class EnrichmentProcessor {
	...
	mergeFromLinkedRecord(base, linked) {
		...
		for (const [k, v] of Object.entries(linked)) {
			...
			if (fieldEntry && isSecondaryContext !== null && typeof fieldEntry.isForSecondary === 'boolean') {
				if (fieldEntry.isForSecondary !== isSecondaryContext) continue;
			}
		...
		const walk = async (v) => {
			...
			if (isObj(jump) && isObj(jump._DBLink)) {
				...
				if (linked) {
					...
					const resolved = this.resolveJumpsInAny(sanitized, linked);
	 ...
	 * - mergedVars.$Def_<rootKey>.#List_<keyField> の一致エントリを検索
	 * - $IndexDef で宣言されたサブフィールド、および主キーフィールドの言語バリアント（_JP / _EN）を
	 *   空値の場合のみ埋める（既存値は維持）
	 *
	 * @param {Object} record - レコード
	 * @param {Object|null} indexDef - $IndexDef オブジェクト
	...
	supplementIndexFieldFromVarsDef(record, indexDef, mergedVars) {
		...
		if (keyValue == null || keyValue === '') return record;

		// $VarsDef.$Def_<rootKey>.#List_<keyField> を参照（例: $Def_Chronos.#List_Lunar）
		const list = mergedVars?.[`$Def_${rootKey}`]?.[`#List_${keyField}`];
		if (!Array.isArray(list)) return record;

		// 主キー値に一致するリストエントリを検索（文字列比較）
		const keyValueStr = String(keyValue).trim();
	...
	async enrichRecords(records, workId, dbName = '') {
		...
		try {
			...
			const enrichedRecords = await Promise.all(records.map(async (record) => {
				...
				// 1) typedef に基づく軽い正規化（#Index 値は解決済み IndexDef で正規化）
				const normalizedRecord = this.normalizeRecordByTypeDef(record, ctx?.defTypeMerged, { indexDef: resolvedIndexDef });
				...
				// 2.1) $enrich: true を持つ *_DBLink suffix フィールドを解決してマージ
				//      typedef の $enrich: true でフィールド単位に制御。先頭の解決済みエントリのみマージ。
				...
				if (allImages.length > 0) {
					...
					enrichedRecord._enrichment.primaryImage = imageInfo.primaryImage || allImages[0]?.url || null;
	...
	async searchRecords(records, workId, dbName, queries) {
		 ...
		 * - 例: 'FormalName_JP' -> ['FormalName_JP','FormalName','FormalName_EN']
...
class TypeDefUtils {
	...
	static looksSearchableType(typeSpec) {
		...
		if (typeof typeSpec === 'string') {
			...
			// - #DictIndex: Area / Belonging など辞書参照フィールド
			// - $Def_Day / $Def_StoryEra* / $Def_BaseArea: Day / Era / Area の構造型
			return /(#String|#Summary|#Dialogue|#Enum|\$EnumDef|#DictIndex|\$Def_Day|\$Def_StoryEra|\$Def_BaseArea)/i.test(typeSpec);
		}
