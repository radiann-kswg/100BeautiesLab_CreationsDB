...
class TypeDefUtils {
	 ...
	 * - 語彙は 4 種のみ: $type（完全一致）/ $typeIncludes（部分一致）/ $display（浅い部分集合一致）/ "*"（catch-all）
	 ...
	 */
	static matchesSlot(match, entry) {
	...
	static sortEntriesByDeclaredOrder(entries, order) {
		...
		const rankOf = (entry) => {
			...
			const base = key.replace(/_(JP|JPReading|EN)$/, '');
	 ...
	 * @param {Object} [options.detailLayout] - 作品の $DetailLayout（`$slotOrder` の解決に使う）
	 * @returns {Array<Object>} merged $DefType（マーカーは含まない）
	 */
	static mergeDefTypes(globalType, workType, options = {}) {
		...
		for (const we of w) {
			...
			const slot = (typeof we.$slot === 'string' && bySlot.has(we.$slot))
				? we.$slot
				: (markers.find(m => m.$slotMatch !== '*' && this.matchesSlot(m.$slotMatch, we))?.$slot ?? fallbackSlot);
