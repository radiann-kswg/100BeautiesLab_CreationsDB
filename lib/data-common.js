...
class EnrichmentProcessor {
  ...
  async getWorkContext(workId) {
    ...
    const ctx = { t: now, mergedVars, defTypeMerged, indices, indexDef };
  ...
  buildEnrichmentIndices(mergedVars, defTypeMerged) {
    const indices = {
      ...
      // varsdef の #List_Belonging に含まれる BaseArea を逆引きする
      belongingBaseAreas: new Map(),
  ...
  async resolveDbLinkPrimaryRecord(dbLinkDef, cache) {
    ...
    const p = (async () => {
      try {
        ...
        const matched = await this.searchRecords(allRecords, workKey, dbName, queries);
  ...
  extractFromVarDefinition(varDef, indices) {
    const visit = (node) => {
      ...
      Object.entries(node).forEach(([key, value]) => {
        ...
        if (key === '#List_Belonging' && Array.isArray(value)) {
          const byBelonging = indices?.belongingBaseAreas instanceof Map ? indices.belongingBaseAreas : null;
          if (byBelonging) {
            for (const item of value) {
              if (!item || typeof item !== 'object' || Array.isArray(item)) continue;
              const baseArea = item.BaseArea;
              if (!baseArea || typeof baseArea !== 'object' || Array.isArray(baseArea)) continue;

              const candidates = [item.Belonging, item.Belonging_JP, item.Belonging_EN]
                .map(v => (typeof v === 'string' ? v.trim() : ''))
                .filter(Boolean);

              for (const candidate of candidates) {
                if (!candidate || byBelonging.has(candidate)) continue;
                byBelonging.set(candidate, { ...baseArea });
              }
            }
          }
        }
  ...
  /**
   * varsdef の #List_Belonging から BaseArea を引き当て、BelongingArea を補助展開する
   * @param {Object} record
   * @param {Object|null} indices
   * @returns {Object}
   */
  deriveBelongingArea(record, indices = null) {
    if (!record || typeof record !== 'object') return record;
    const byBelonging = indices?.belongingBaseAreas instanceof Map ? indices.belongingBaseAreas : null;
    if (!byBelonging || byBelonging.size === 0) return record;
    ...
    if (!isEmpty(record.BelongingArea)) return record;

    const entries = Array.isArray(record.Belonging)
      ? record.Belonging
      : (typeof record.Belonging === 'undefined' ? [] : [record.Belonging]);
    ...
    const pushArea = (area) => {
      ...
      const dedupeKey = JSON.stringify(area);
    ...
    for (const entry of entries) {
      ...
      if (typeof entry === 'string') {
        ...
        const raw = typeof entry.Belonging === 'string' ? entry.Belonging.trim() : '';
        const jp = typeof entry.Belonging_JP === 'string' ? entry.Belonging_JP.trim() : '';
        const en = typeof entry.Belonging_EN === 'string' ? entry.Belonging_EN.trim() : '';
        if (raw) candidates.push(raw);
        if (jp) candidates.push(jp);
        if (en) candidates.push(en);
      }

      for (const candidate of candidates) {
        const area = byBelonging.get(candidate);
        if (area) pushArea(area);
      }
    }
    ...
    const out = { ...record };
    out._enrichment = out._enrichment || {};
    out._enrichment.derivedBelongingAreas = derived;
    if (derived.length === 1) {
      out.BelongingArea = derived[0];
    }
    return out;
  }
  ...
  async enrichRecords(records, workId, dbName = '') {
    ...
    try {
      ...
      const enrichedRecords = await Promise.all(records.map(async (record) => {
        ...
        // 2.8) #List_Belonging にネストされた BaseArea から BelongingArea を補助展開
        enrichedRecord = this.deriveBelongingArea(enrichedRecord, ctx?.indices || null);
...
class TypeDefUtils {
  ...
  static extractDefTypeEntries(typeDef) {
    ...
    if (Array.isArray(typeDef?.$TypeDef)) return typeDef.$TypeDef.filter(it => it && typeof it === 'object');
