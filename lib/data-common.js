...
class EnrichmentProcessor {
  ...
  async getWorkContext(workId) {
    ...
    // VarsDef: meta(General.$VarsDef) と type($VarsDef) の両方を対象にする
    const [globalVarsMeta, workVarsMeta, globalType, workType] = await Promise.all([
    ...
    const ctx = { t: now, mergedVars, defTypeMerged, indices };
  ...
  async enrichRecords(records, workId, dbName = '') {
    ...
    try {
      // typedef/varsdef を読み込み、挙動をスキーマ追従にする
      ...
      const enrichedRecords = records.map(record => {
        ...
        const enrichedRecord = { ...normalizedRecord };

        // 画像情報を処理
        const imageInfo = imageProcessor.imageFromRecord(normalizedRecord, workId, dbName, ctx?.indices?.imagePathHints);
        ...
        // 検索可能フィールドのインデックス化
        enrichedRecord._enrichment = enrichedRecord._enrichment || {};
        enrichedRecord._enrichment.searchableText = this.buildSearchableText(normalizedRecord, ctx?.indices);

        // 表示分類（typedef 駆動）
        const displaySections = this.buildDisplaySections(normalizedRecord, ctx?.defTypeMerged, ctx?.indices);
        if (displaySections) {
  ...
  normalizeRecordByTypeDef(record, defTypeMerged) {
    ...
    for (const entry of entries) {
      ...
      const typeSpec = entry?.$type;
      out[key] = TypeDefUtils.normalizeValueByTypeSpec(out[key], typeSpec);
    }
    return out;
  }
  ...
  async searchRecords(records, workId, dbName, queries) {
    ...
    const normalizeQueryKey = (hashTag, rawKey) => {
      const typeSpec = typeByKey.get(hashTag);
    ...
    const valueEquals = (val, qKey, typeSpec) => {
      ...
      if (TypeDefUtils.looksNumberType(typeSpec)) {
        const a = typeof val === 'number' ? val : (typeof val === 'string' ? parseFloat(val) : NaN);
        const b = typeof qKey === 'number' ? qKey : (typeof qKey === 'string' ? parseFloat(qKey) : NaN);
        if (!Number.isNaN(a) && !Number.isNaN(b)) return a === b;
      }

      return String(val) === String(qKey);
    };

    return records.filter(rec => {
      return normalizedQueries.every(q => {
        const typeSpec = typeByKey.get(q.hashTag);
        const val = getByPath(rec, q.hashTag);
        return valueEquals(val, q.key, typeSpec);
      });
    });
  }
}
...
class TypeDefUtils {
  ...
  static pickDisplaySection(entry) {
    ...
    if (typeStr.includes('#Summary') || /(Calling|Character|Hobby|SpetialSkill|Favor|Unlike|About|Comment|Summary)/i.test(key)) return 'profile';
  ...
  static looksSearchableType(typeSpec) {
    if (!typeSpec) return false;
    if (typeof typeSpec === 'string') {
      return /(#String|#Stiring|#Summary|#Enum|\$EnumDef)/i.test(typeSpec);
    }
  ...
  static looksNumberType(typeSpec) {
    if (!typeSpec) return false;
    if (typeof typeSpec === 'string') return /#Number/i.test(typeSpec);
    return false;
  }
  ...
  static normalizeValueByTypeSpec(value, typeSpec) {
    ...
    // #String/#Summary 系はプリミティブを string に寄せる
    if (typeof typeSpec === 'string' && /(#String|#Stiring|#Summary)/i.test(typeSpec)) {
