...
class EnrichmentProcessor {
  ...
  async getWorkContext(workId) {
    ...
    // work の index 定義（グローバルメタ）
    // - 既定: $DefType_Index（新）
    // - 後方互換: $Def_Index（旧）
    const workMeta = globalMeta?.CreationWorks?.[workId] ?? null;
    const indexDef = workMeta?.$DefType_Index ?? workMeta?.$Def_Index ?? null;
  ...
  async searchRecords(records, workId, dbName, queries) {
    ...
    // work の $DefType_Index（data/db_meta.json）を、#Index の解釈に利用
...
class TypeDefUtils {
  ...
  static normalizeValueByTypeSpec(value, typeSpec) {
    ...
    // #Index は、作品の $DefType_Index がネスト型の場合のみ、最低限の形を補正
