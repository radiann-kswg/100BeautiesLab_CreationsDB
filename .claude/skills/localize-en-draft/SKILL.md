...
description: data/Works_*/DataBases/db_*.json 等の JP フィールド（Summary_JP・Character_JP 等）から、対応する空の _EN フィールドを docs/localization-en-rules.md のルールに従って下書き翻訳する。DeepL API や外部コネクタを使わず、Claude 自身がこの手順で翻訳する。field_EN キー自体が未追加（新規挿入が必要）なケースや、少数レコードを丁寧に訳したいときに使う。「Summary_ENを埋めて」「〜の英訳をお願い」「_ENが空だから埋めて」等のときに使う。
---

# localize-en-draft — 英訳（`_EN`）下書き翻訳

`data/` 配下 JSON の日本語フィールドから、対応する英語フィールド（`_EN`）を Claude 自身が翻訳して埋めるための手順書。DeepL の MCP コネクタは対話セッション専用のツールでスクリプトから呼び出せないため、「Claude 自身が翻訳する」運用をこの Skill として型化したもの。
...
8. **報告**: 何件・どのレコードのどのフィールドを埋めたかをユーザーに提示する。訳に迷いがある箇所は確定させず候補として提示する。`CLAUDE.md` の運用ルールに従い、大規模編集（多数ファイル・多数レコード）の場合は `_work_in_progress/` に進捗ログを残すことも検討する。
